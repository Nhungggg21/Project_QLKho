...
class LoaiNguyenLieuController extends Controller
{
    ...
    {
        try {
            // Kiểm tra nếu loại nguyên liệu này đang được sử dụng bởi nguyên liệu
            $LoaiNguyenLieu = LoaiNguyenLieu::find($id);
    
            if (!$LoaiNguyenLieu) {
                Alert::error('Thất bại', 'Loại nguyên liệu không tồn tại.');
                return back();
            }
    
            // Kiểm tra sự tồn tại của nguyên liệu trong loại nguyên liệu này
            if ($LoaiNguyenLieu->NguyenLieu()->exists()) {
                Alert::error('Thất bại', 'Không thể xóa loại nguyên liệu này vì đang có nguyên liệu liên quan.');
                return back();
            }
    
            // Thực hiện xóa
            $status = $LoaiNguyenLieu->delete();
